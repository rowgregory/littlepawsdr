...
const updateAuctionItem = asyncHandler(async (req, res) => {
  try {
    const auctionItem = await AuctionItem.findById(req.body.id).populate('photos').populate('auction');
    if (!auctionItem) return res.status(404).json({ message: 'Auction item not found' });

    // Check if auction is live
    const isAuctionLive = auctionItem.auction?.settings?.status === 'LIVE';
    ...
    if (isAuctionLive) {
      const attemptedRestrictedUpdates = restrictedFields.filter((field) => req.body[field] !== undefined && field !== 'photos');

      if (attemptedRestrictedUpdates.length > 0) {
        return res.status(403).json({
          message: 'Cannot update pricing, quantity, or format fields while auction is live. You can only update name, description, and photos.',
    ...
    const updatedAuctionItem = await AuctionItem.findByIdAndUpdate(auctionItem._id, updateData, { new: true }).populate('photos');

    res.status(200).json({
      auctionItem: updatedAuctionItem,
      message: isAuctionLive ? 'Auction item updated (limited fields during live auction)' : 'Auction item updated',
      sliceName: 'campaignApi',
    });
  } catch (err) {
    await Error.create({
