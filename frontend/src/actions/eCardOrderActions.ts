...
export const listMyEcardOrders = () => async (dispatch: any, getState: any) => {
  try {
    dispatch({ type: ECARD_ORDER_LIST_MY_REQUEST });
    ...
    const { data } = await axios.get(`/api/ecard-order`, config);

    dispatch({ type: ECARD_ORDER_LIST_MY_SUCCESS, payload: data });
  } catch (error: any) {
    dispatch({
      type: ECARD_ORDER_LIST_MY_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
