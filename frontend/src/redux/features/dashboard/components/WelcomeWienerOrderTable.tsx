...
import useSortableArray from '../../../../utils/hooks/useSortableArray';
...
const WelcomeWienerOrderTable = () => {
  ...
  const [sortedArray, sortColumn] = useSortableArray(dashboard?.welcomeWienerOrders ?? []);

  const handleSortColumn = (col: string) => sortColumn(col);

  return noWelcomeWienerOrders ? (
    <div className='flex flex-col justify-center max-w-52 w-full items-center mx-auto  py-10'>
      ...
      <div className='font-Matter-Medium my-2'>No welcome wiener orders</div>
    </div>
  ) : isLoading ?
    ...
    : (
      <Fragment>
        <div className='flex flex-col bg-white p-3 rounded-xl'>
          <p className='text-sm font-Matter-Light text-gray-500 mb-2'>Welcome Wiener Order Summary</p>
          <div className="flex justify-between items-baseline">
            <div className="flex  items-baseline">
              <h1 className='font-Matter-Medium text-2xl mr-2'>${toFixed(dashboard?.welcomeWienerRevenue)}</h1>
              <p className='font-Matter-Light text-gray-500 text-sm mr-2'>{dashboard?.totalWelcomeWieners} orders</p>
            </div>
          </div>
        </div>
        <div className='rounded-xl bg-white overflow-x-scroll lg:overflow-x-hidden relative'>
          <table className='w-full'>
            <thead className='whitespace-nowrap px-4 pb-4 pt-2'>
              <tr className='bg-zinc-50'>
                ...
                <th onClick={() => handleSortColumn('price')} className='px-4 border-b border-gray-100 font-Matter-Regular text-star py-2'>
                  <div className='text-sm cursor-pointer -mx-1.5 -my-1 w-fit px-1.5 py-1 rounded-md hover:bg-gray-200 flex flex-nowrap items-center gap-2'>
                    Price
                  </div>
                </th>
          ...
          <p className='font-Matter-Light text-gray-600 text-xs px-3 pb-3 pt-8'>First order created on {formatDateWithTimezone(dashboard?.firstWelcomeWienerOrderCreatedAt)}</p>
        </div>
      </Fragment>
    );
};
export default WelcomeWienerOrderTable