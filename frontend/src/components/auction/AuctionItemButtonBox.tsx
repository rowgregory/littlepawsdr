...
import { IAuctionItemButtonBox, SparkleData } from '../../types/auction-types';
import { setAuctionItem, setOpenBidModal } from '../../redux/features/auctionSlice';
...
const AuctionItemButtonBox: FC<IAuctionItemButtonBox> = ({
  ...
  if (customAuctionLink) {
    params.set('customAuctionLink', customAuctionLink);
  }
  if (auctionItem) {
  ...
  if (ifAuctionIsOver && auctionItem?.sellingFormat === 'auction' && auctionItem.totalBids > 0) {
    return (
        ...
        >
          <div className='relative bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-500 hover:via-purple-500 hover:to-pink-500 text-white font-bold text-2xl rounded-2xl py-6 px-8 text-center transform hover:scale-105 hover:-translate-y-1 transition-all duration-300 shadow-2xl hover:shadow-purple-500/50 border-2 border-purple-400/50 overflow-hidden'>
            ...
            <div className='relative z-10 flex items-center justify-center gap-3'>
              ...
              <span>
                ✨ Register to {auctionItem?.sellingFormat === 'auction' ? 'Bid' : 'Buy Now'} ✨
              </span>
            </div>
          </div>
        </Link>
  ...
  if (user?._id && !user?.hasAddress) {
    return (
      <div className='relative mb-6'>
        <ButtonSparkle isActive={isHovered} />
        <Link
          to='/supporter/profile?initialLogin=true'
          className='block hover:no-underline'
          onMouseEnter={() => setIsHovered(true)}
          onMouseLeave={() => setIsHovered(false)}
        >
  ...
  if (auctionItem?.sellingFormat === 'fixed') {
    const isDisabled: boolean =
      auction?.status === 'DRAFT' || ifAuctionIsOver || auctionItem?.totalQuantity === 0;
    ...
    return (
      <div className='relative mb-6'>
        ...
        <button
          ...
          onClick={() => {
            triggerLightning(() => {
              navigate(`/auctions/${customAuctionLink}/item/${auctionItem?._id}/buy`);
            });
          }}
        >
  ...
  if (!ifAuctionIsOver) {
    const isDisabled: boolean = auction?.status === 'DRAFT' || ifAuctionIsOver;
