...
const WelcomeDrawer: React.FC<WelcomeDrawerProps> = ({ userName = 'Greg' }) => {
  ...
  useEffect(() => {
    ...
    const customCampaignLink = urlParams.get('customCampaignLink');
    ...
    if (customCampaignLink) {
      ...
      setAuctionData({ customCampaignLink });
      if (auctionItemId) {
    ...
    if (isInitialLogin) {
      ...
      const newUrl = window.location.pathname + window.location.search.replace(/[?&]initialLogin=true/, '').replace(/^&/, '?');
    ...
    const baseSteps: Step[] = [
      {
        ...
        description: "Thanks for creating your account. We're excited to have you join our community!",
        color: 'from-purple-500 to-pink-500',
      },
      {
        icon: User,
        title: 'Complete Your Profile',
        description: 'This is your personal dashboard where you can manage all your information in one place.',
        color: 'from-blue-500 to-cyan-500',
      },
      {
        icon: Shield,
        title: 'Stay Secure',
        description: 'Your privacy and security are our top priority. All your data is encrypted and protected.',
      ...
      return [
        {
          ...
          description: 'Complete your shipping address to participate in the auction and place bids on items.',
  ...
  const nextStep = (): void => {
    if (currentStep < steps.length - 1) {
      ...
      if (hasAuctionParams && auctionData.customCampaignLink && auctionData.auctionItemId) {
        window.location.href = `/campaigns/${auctionData.customCampaignLink}/auction/item/${auctionData.auctionItemId}`;
      }
      if (hasAuctionParams && auctionData.customCampaignLink) {
        window.location.href = `/campaigns/${auctionData.customCampaignLink}/auction`;
      }
    }
  };
    ...
    <div className='space-y-4 mb-8'>
      <div className='flex items-start space-x-3 p-4 bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200 rounded-xl'>
        ...
        <div>
          ...
          <p className='text-sm text-gray-700 leading-relaxed'>
            You'll need a complete shipping address to participate in auctions. This ensures smooth delivery if you win!
          </p>
        </div>
      </div>
      ...
      <div className='flex items-start space-x-3 p-4 bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-xl'>
        ...
        <div>
          ...
          <p className='text-sm text-gray-700 leading-relaxed'>Once your address is complete, you can place bids and compete for amazing items!</p>
        </div>
      </div>
    ...
    <>
      ...
      >
        ...
        <div className='relative overflow-hidden'>
          <div className={`absolute inset-0 bg-gradient-to-br ${steps[currentStep].color} opacity-10`}></div>
          <div className='relative px-6 py-8'>
            <button onClick={closeDrawer} className='absolute top-4 right-4 p-2 hover:bg-gray-100 rounded-full transition-colors duration-200'>
              <X className='w-5 h-5 text-gray-500' />
            </button>

            <div className='text-center'>
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br ${steps[currentStep].color} mb-4`}>
        ...
        <div className='px-6 py-4'>
            ...
            {steps.map((_, index) => (
              <div
                ...
                className={`w-2 h-2 rounded-full transition-colors duration-200 ${
                  index === currentStep ? (steps[currentStep].isAuctionStep ? 'bg-orange-500' : 'bg-slate-900') : 'bg-gray-300'
                }`}
              />
            ))}
          </div>
          ...
          <div className='space-y-3'>
            ...
            >
              <span>
                {steps[currentStep].isAuctionStep ? 'Complete Profile for Auction' : currentStep < steps.length - 1 ? 'Next' : 'Get Started'}
              </span>
              <ArrowRight className='w-4 h-4' />
            </button>

            <button onClick={closeDrawer} className='w-full text-gray-600 hover:text-gray-800 py-2 font-medium transition-colors duration-200'>
