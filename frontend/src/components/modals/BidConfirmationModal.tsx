...
const BidConfirmationModal = ({ openConfirmationModal, handleCloseConfirmationModal, campaign, setOpenConfirmationModal }: any) => {
  const handleClose = () => {
    setOpenConfirmationModal(false);
    if (handleCloseConfirmationModal) {
      handleCloseConfirmationModal();
    }
  };

  return (
    <Modal show={openConfirmationModal} onClose={handleClose}>
      <motion.div
        className='relative max-w-md w-full mx-auto p-4'
        initial={{ scale: 0.8, opacity: 0, y: 50 }}
        animate={{
          scale: openConfirmationModal ? 1 : 0.8,
          opacity: openConfirmationModal ? 1 : 0,
          y: openConfirmationModal ? 0 : 50,
        }}
          ...
          <div className='relative z-10 text-center space-y-6'>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
              ...
              <p className='text-white/70 text-sm leading-relaxed'>
                Your bid has been successfully placed. You can track your bids in your profile dashboard.
              </p>
            </motion.div>
            ...
            >
              ...
              <p className='text-3xl font-bold text-white'>${campaign?.confirmedBidAmount?.toLocaleString() ?? '0'}</p>
            </motion.div>
