...
const AuctionDonationCard = ({ donation, theme }: { donation: any; theme: any }) => {
  return (
    <div className='border-[0.5px] border-gray-200 p-3.5 rounded-sm flex flex-col mb-4 fade-in'>
      <div className='grid grid-cols-9 gap-1 mb-2 items-center'>
        ...
        <p className='col-span-7 md:col-span-12 lg:col-span-7 font-Matter-Light'>
          <span className='font-Matter-Medium'>
            {formatNameForAuctionItemCard(donation?.donor)}
          </span>{' '}
          made a <span className='font-Matter-Medium'>${donation?.oneTimeDonationAmount}</span>{' '}
          donation
        </p>
      </div>
