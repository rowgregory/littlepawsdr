...
const AuctionItemWinnerContainer: React.FC<AuctionItemWinnerContainerProps> = ({ winningBid, setOrderLoader, payForAuctionItem }) => {
      ...
      <div className='min-h-[calc(100dvh-128px)] flex items-center justify-center p-4'>
        <div className='max-w-md mx-auto text-center'>
          ...
          >
            ...
            <p className='text-white/80 text-lg mb-6'>Thank you for your payment. Your items will be shipped soon!</p>
            ...
            <motion.div className='space-y-8' initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              ...
              <div className='text-center'>
                ...
                >
                  ...
                  {/* Floating stars */}
                  <motion.div className='absolute -top-2 -left-2' animate={{ rotate: [0, 360] }} transition={{ duration: 4, repeat: Infinity }}>
                    <Star className='w-4 h-4 text-yellow-300' />
                  </motion.div>
                  <motion.div className='absolute -bottom-2 -right-2' animate={{ y: [-2, 2, -2] }} transition={{ duration: 2, repeat: Infinity }}>
                    <Sparkles className='w-4 h-4 text-yellow-300' />
                  </motion.div>
                </motion.div>

                <h2 className='text-5xl font-bold text-white mb-4'>{winningBid?.user?.name || 'Congratulations!'}</h2>
              ...
              >
                <div className='flex items-center gap-3 mb-6'>
                  ...
                  <h3 className='text-2xl font-bold text-white'>
                    Your Winning Item{winningBid?.auctionItems && winningBid.auctionItems.length > 1 ? 's' : ''}
                  </h3>
                </div>
                    ...
                    winningBid.auctionItems.map((item: any, index: number) => (
                      ...
                      >
                        <div className='flex items-center gap-6'>
                          ...
                          />
                          <div>
                            <h4 className='font-bold text-white text-xl mb-2'>{item?.title || 'Auction Item'}</h4>
                            <p className='text-white/70 text-sm mb-1'>
                              Item Price: <span className='text-yellow-400 font-semibold'>${item?.soldPrice || '0.00'}</span>
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='border-t border-white/20 pt-4'>
                      <div className='flex justify-between text-white font-bold text-2xl'>
                        ...
                        <span className='text-yellow-400'>${toFixed(winningBid?.totalPrice) || '0.00'}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <p className='text-white/90 leading-relaxed'>
                  Your purchase directly helps rescue, rehabilitate, and rehome dachshunds in need. Thank you for making a difference in these
                  precious lives!
                </p>
              </motion.div>
            </motion.div>
            ...
            >
              ...
              <div className='bg-gradient-to-br from-indigo-800/50 to-purple-800/50 backdrop-blur-xl border border-white/20 rounded-3xl p-8 sticky top-8'>
                <div className='text-center mb-8'>
                  ...
                  <div className='mt-4 text-3xl font-bold text-yellow-400'>${toFixed(winningBid?.totalPrice) || '0.00'}</div>
                </div>

                {/* Trust Indicators */}
                <div className='grid grid-cols-3 gap-4 mb-8'>
                  <motion.div className='text-center p-4 bg-white/10 rounded-xl border border-white/10' whileHover={{ scale: 1.05 }}>
                ...
                {/* PayPal Buttons */}
                <motion.div className='bg-white rounded-2xl p-6' whileHover={{ y: -2 }} transition={{ duration: 0.3 }}>
                ...
                <div className='mt-6 pt-6 border-t border-white/20'>
                  ...
                  <p className='text-center text-white/40 text-xs mt-2'>Your payment information is secure and encrypted</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <div className='lg:hidden px-4 py-8'>
        <div className='max-w-md mx-auto space-y-6'>
          ...
          <motion.div className='text-center' initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h2 className='text-3xl font-bold text-white mb-2'>{winningBid?.user?.name || 'Congratulations!'}</h2>
          ...
          >
            ...
            <div className='space-y-3 mb-6'>
              {winningBid?.auctionItems && winningBid.auctionItems.length > 0 ? (
                winningBid.auctionItems.map((item: any, index: number) => (
                  <div key={item._id || index} className='flex items-center gap-4 p-4 bg-white/10 rounded-lg'>
                    <img
                    ...
                    <div className='flex-1'>
                      <h4 className='font-semibold text-white text-lg mb-1'>{item?.title || 'Auction Item'}</h4>
          ...
          >
            <div className='text-center mb-6'>
              ...
              <div className='mt-2 text-2xl font-bold text-yellow-400'>${toFixed(winningBid?.totalPrice) || '0.00'}</div>
            </div>
