...
import { closeAdminMobileNavigation } from '../../redux/features/dashboardSlice';
...
const AdminMobileNavigationDrawer = () => {
  ...
  const onClose = () => dispatch(closeAdminMobileNavigation());
  ...
  const handleLogout = async () => {
    try {
      ...
      dispatch(showToast({ message: 'An error occurred while logging out', type: 'error' }));
    }
  };
  ...
  const linkState = {
    ...
    auctions: pathname.includes('/auctions'),
    orders: pathname.includes('/orders'),
    donations: pathname.includes('/donations'),
