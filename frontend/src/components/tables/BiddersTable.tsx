    ...
    <table className='w-full'>
      <thead className='whitespace-nowrap px-4 pb-4 pt-2'>
        <tr className='bg-zinc-50'>
          ...
          <th onClick={() => handleSort('user.email')} className='px-4 border-b border-gray-100 font-Matter-Regular py-2'>
            <div className=' text-sm flex flex-nowrap items-center gap-2 cursor-pointer -mx-1.5 -my-1 w-fit px-1.5 py-1 rounded-md hover:bg-gray-200'>
              Email
            </div>
          </th>
          <th onClick={() => handleSort('bids')} className='px-4 border-b border-gray-100 font-Matter-Regular py-2'>
            <div className=' text-sm flex flex-nowrap items-center gap-2 cursor-pointer -mx-1.5 -my-1 w-fit px-1.5 py-1 rounded-md hover:bg-gray-200'>
              Bids
            </div>
          </th>
          <th onClick={() => handleSort('user._id')} className='px-4 border-b border-gray-100 font-Matter-Regular py-2'>
            <div className=' text-sm flex flex-nowrap items-center gap-2 cursor-pointer -mx-1.5 -my-1 w-fit px-1.5 py-1 rounded-md hover:bg-gray-200'>
              Bidder ID #
            </div>
          </th>
        ...
        {filteredData?.map((bidder: any, i: number) => (
          <Fragment key={i}>
            ...
            >
              <td className='w-10 h-8'>
                <i
                  className={`text-gray-800 fas fa-chevron-right ml-4 duration-300 origin-center ${
                    currentBidder?.user?._id === bidder?.user?._id && currentBidder?.open ? 'rotate-90' : ''
                  }`}
                ></i>
              </td>
              <td>
              ...
              <td>
                <p className='text-gray-800 text-sm font-Matter-Regular items-center px-4 whitespace-nowrap'>{bidder?.user?.email}</p>
              </td>
              <td>
                <p className={`text-gray-800 text-sm font-Matter-Regular items-center px-4 whitespace-nowrap`}>{bidder?.bids?.length}</p>
              </td>
              <td>
                <p className='text-gray-800 text-sm font-Matter-Regular items-center px-4 whitespace-nowrap'>{bidder?.user?._id}</p>
              </td>
            </tr>
                ...
                <td colSpan={5}>
                  <div className='grid grid-cols-12 px-4 py-3 gap-4'>
                    <div className='col-span-6'>
                      ...
                      <div className='flex flex-col px-2 py-1.5 rounded-sm gap-y-3 h-40 overflow-y-auto'>
                        {bidder?.bids
                          ?.map((bid: any, i: number) => (
                            <div key={i} className='flex items-center justify-between'>
                              <div className='flex gap-x-1.5'>
                                <img src={bid.auctionItem.photos[0].url} alt='Lpdr' className='w-12 h-12 rounded-md object-cover' />
                                <div className='flex flex-col'>
                                  <h2 className='text-sm text-zinc-800 truncate max-w-72'>{bid?.auctionItem?.name}</h2>
                                  <div className='text-gray-600 text-xs'>Bid Amount: ${bid?.bidAmount}</div>
                                  <div className='text-gray-600 text-[10px]'>{formatDateWithTimezone(bid?.createdAt)}</div>
                                </div>
                              </div>
                              <p
                                className={`${
                                  bid?.status === 'Top Bid' ? 'text-green-500 bg-green-50' : 'text-amber-500 bg-amber-50'
