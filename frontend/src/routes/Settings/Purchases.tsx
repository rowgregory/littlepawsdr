...
const Purchases = () => {
  ...
  return noOrders ? (
    <div className='flex flex-col justify-center w-full items-center mx-auto py-10 bg-[#fff] border border-gray-100 rounded-lg'>
      ...
      <Link className='hover:no-underline font-Matter-Regular text-teal-500 hover:text-teal-600 text-center' to='/store'>
        Visit Store
      </Link>
    </div>
  ) : (
            ...
            {orders?.map((item: any, h: number) => (
              <Fragment key={item?._id}>
                  ...
                  <td>
                    <div className='m-0 w-full  p-0 decoration-inherit hover:text-inherit hover:decoration-inherit !flex h-[3.25rem] items-center px-4 whitespace-nowrap'>
                      <div className='max-w-[15rem]'>
                        <span className='text-sm font-Matter-Regular truncate'>${item?.totalPrice?.toFixed(2)}</span>
                      </div>
                    </div>
                  </td>
                  <td>
                ...
                {openRows?.rows.map(
                  (row: any) =>
                    row.id === item._id && (
                      <tr key={row?.id} className={`${h !== orders.length - 1 ? 'border-b-2 border-slate-200' : 'border-none'}`}>
                        <td colSpan={12} className='pt-3'>
                          <div className='grid grid-cols-12 px-4 pt-2 pb-4 gap-8'>
                            <div className='col-span-6'>
                              ...
                              {[...(row.products || []), ...(row.ecards || []), ...(row.welcomeWieners || [])]?.map((item: any, j: number) => (
                                <div key={j} className='flex items-center justify-between py-1.5'>
                                      ...
                                      <div className='font-Matter-Light text-sm'>
                                        {item?.type === 'ECARD' && (
                                          <Fragment>
                                            {item?.isSent ? (
                                              <div>
                                                <div className='text-green-500 w-fit text-xs bg-green-100 rounded-2xl px-2 py-0.5'>Delivered</div>
                                                <div className='text-xs'>
                                                  Sent to {item?.recipientsFullName} on {format(item?.dateToSend, 'MMM do, yyyy')}
                                                </div>
                                              </div>
                                            ) : (
                                              <div>
                                                <div className='text-amber-500 w-fit text-xs bg-amber-100 rounded-2xl px-2 py-0.5'>Queued</div>
                                                <div className='text-xs'>
                                                  Sending to {item?.recipientsEmail} on {convertToEST(item?.dateToSend)}
                                                </div>
                                              </div>
                                            )}
                                          </Fragment>
                                        ...
                                        {item?.type === 'WELCOME_WIENER' && (
                                          <div>
                                            <div className='text-teal-500 w-fit text-xs bg-teal-100 rounded-2xl px-2 py-0.5'>Processed</div>
                                            <div className='text-xs'>
                                              {item?.quantity} x {item?.productName} for {item?.dachshundName}
                                            </div>
                                          </div>
                                        )}
                                      </div>
                                        ...
                                        <div>
                                          <div
                                            className={`w-fit text-xs rounded-2xl px-2 py-0.5 ${
                                              item?.status === 'Not Shipped' ? 'bg-red-100 text-red-500' : 'bg-green-100 text-green-500'
                                            }`}
                                          >
                                            {item?.status}
                                          </div>
                                  ...
                                  <p className='font-Matter-Light text-sm'>${toFixed(item?.totalPrice || item?.price)}</p>
                                </div>
                              ))}
                              <div className='px-4 border-b-[1px] border-gray-200 w-full my-3.5'></div>
                              {row?.products?.length >= 1 && (
                                <div className='grid grid-cols-8'>
                                  <p className='text-sm font-Matter-Light col-start-4 col-span-3 text-right'>Shipping Costs</p>
                                  <p className='text-sm font-Matter-Light col-start-8 col-span-1 text-right'>${toFixed(row.shippingPrice)}</p>
                                </div>
                              )}
                              <div className='grid grid-cols-8 mt-1'>
                                <p className='text-sm font-Matter-SemiBold col-start-4 col-span-3 text-right'>Total Price</p>
                                <p className='text-sm font-Matter-SemiBold col-start-8 col-span-1 text-right'>${toFixed(row.totalPrice)}</p>
                              </div>
                              <div
                            ...
                            <div className='col-span-6'>
                              ...
                              {row.isShipped && (
                                <div className='grid grid-cols-12 mt-1'>
                                  <p className='text-sm font-Matter-Light col-span-3 text-left whitespace-nowrap'>Shipped Date</p>
                                  <p className='text-sm font-Matter-SemiBold col-span-8'>{formatDateWithTimezone(row.shippedOn)}</p>
                                </div>
                              )}
                              <div className='grid grid-cols-12 mt-1'>
                                <p className='text-sm font-Matter-Light col-span-3 text-left'>Order Id</p>
                                <p className='text-sm font-Matter-SemiBold'>{row.id}</p>
                              </div>
                              <div className='grid grid-cols-12 mt-1 mb-3.5'>
                                <p className='text-sm font-Matter-Light col-span-3 text-left'>PayPal Id</p>
                                <p className='text-sm font-Matter-SemiBold'>{row.paypalOrderId}</p>
                              </div>
                              {row?.status === 'Complete' && (
                                <Fragment>
                                  <label className='font-Matter-Medium mb-1' htmlFor='Tracking number'>
                                    Tracking Number
                                  </label>
                                  <div
                                    onClick={() =>
                                      window.open(`https://www.google.com/search?q=${row.shippingProvider}%3A+${row.trackingNumber}`, '_blank')
                                    }
