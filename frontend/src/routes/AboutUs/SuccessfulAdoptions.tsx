...
import { Container, DogContainer, LoadMoreBtn } from '../../components/styles/GridDogStyles';
    ...
    <>
      ...
      <Container>
        <div className='w-100 d-flex justify-content-between mt-3'>
          <LeftArrow text='Home' url='/' text2='Not Available for Adoption Yet' url2='/about/hold' />
          <RightArrow text='Rainbow Bridge' url='/about/rainbow-bridge' />
        </div>
        <Text fontSize='32px' marginTop='56px' fontWeight={400} className='mb-3 mx-auto text-center'>
        ...
        <Text className='mb-3 mt-4 text-center' fontSize='16px'>
          Share your updates and photos of your adopted Little Paws doxie with us on our social media pages!
        </Text>
        { }
        <DogContainer>
          {loading
            ? [...Array(3).keys()].map((_: any, i: number) => <LoadingImg key={i} w='100%' mw='300px' />)
            : displayedData?.map((dachshund: any) => (
              <DachshundCard key={dachshund.id} dachshund={dachshund} type='successful-adoptions' />
            ))}
        </DogContainer>
