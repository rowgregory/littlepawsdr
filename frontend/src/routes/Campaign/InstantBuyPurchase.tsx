...
import { useCreateInstantBuyMutation, useGetAuctionItemQuery } from '../../redux/services/campaignApi';
import { useState } from 'react';
import toFixed from '../../utils/toFixed';
import { Link } from 'react-router-dom';
import InstantBuyPurchaseSuccessModal from '../../components/modals/InstantBuyPurchaseSuccessModal';
import { AlertTriangle, ArrowLeft, ArrowRight, Clock, CreditCard, Heart, Lock, LogIn, ShieldCheck, Sparkles, User, X, Zap } from 'lucide-react';
          ...
          {!instantBuy && (
            <div className='text-center mb-8'>
              <div className='bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl p-8 mb-6'>
                ...
                <p className='text-xl text-white/80'>Secure this item now — it&apos;s fast and easy</p>
              </div>
            </div>
          )}
              ...
              <div className='bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl p-6'>
                ...
                <div className='flex items-start space-x-4 mb-6'>
                  <div className='w-24 h-24 rounded-xl overflow-hidden bg-white/5'>
                    <img src={auctionItem?.photos[0]?.url} className='w-full h-full object-cover' alt='Auction Item' />
                  </div>
                  <div className='flex-1'>
                    <h3 className='text-xl font-bold text-white mb-2'>{auctionItem?.name}</h3>
                    <p className='text-white/70 text-sm mb-3'>Premium auction item from our exclusive collection. Available for instant purchase.</p>
                  ...
                  <div className='bg-white/5 rounded-xl p-4'>
                    ...
                    <div className='space-y-2 text-sm'>
                      <div className='flex justify-between text-white/70'>
                        ...
                        <span className='font-semibold text-white'>${toFixed(auctionItem?.buyNowPrice)}</span>
                      </div>
                      {auctionItem?.requiresShipping && (
                        <div className='flex justify-between text-white/70'>
                          <span>Shipping Fee:</span>
                          <span className='font-semibold text-white'>${toFixed(auctionItem?.shippingCosts)}</span>
                        </div>
                      )}
