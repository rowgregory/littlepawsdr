...
const AuctionItem = () => {
  ...
  const timeUntilEndDate = useTimeUntil(campaign?.campaign?.auction?.settings?.endDate);
  const timeUntilStartDate = useTimeUntil(campaign?.campaign?.auction?.settings?.startDate);
  ...
  const auctionItem = useMemo(() => campaign?.campaign?.auction?.items?.find((item: any) => item._id === auctionItemId), [campaign, auctionItemId]);

  // Add navigation logic with looping
  const { prevItem, nextItem, currentIndex, totalItems } = useMemo(() => {
    const allItems = campaign?.campaign?.auction?.items || [];
  ...
  }, [campaign, auctionItemId]);

  const navigateToItem = (itemId: string) => {
    navigate(`/campaigns/${customCampaignLink}/auction/item/${itemId}`);
  };

  const latestBids = campaign?.campaign?.auction?.bids;
  ...
  useEffect(() => {
    if (campaign?.placeBidSuccess) {
      triggerPop();
      setTimeout(() => {
        dispatch(resetPlaceBidSuccess());
      }, 3000);
    }
  }, [latestBids, campaign?.placeBidSuccess, dispatch]);
  ...
  return (
    <>
      ...
      <div className='min-h-dvh bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 text-white relative pb-40'>
        ...
        <div className={`${campaign?.campaign?.auction?.items?.length >= 2 ? 'hidden lg:block' : 'hidden'}`}>
          {prevItem && (
            <button
          ...
          <div className='max-w-7xl mx-auto px-3 sm:px-6 py-4'>
            <div className='flex flex-col gap-y-3 sm:gap-y-0 sm:flex-row sm:items-center sm:justify-between'>
              ...
              <div className='flex items-center space-x-3'>
                ...
                <Link to='/campaigns' className='px-4 py-2 text-white/90 hover:text-white hover:bg-white/10 rounded-full transition-all'>
                  Campaigns
                </Link>
                <Link
                  to={
                    user?.user?._id
                      ? '/settings/profile'
                      : `/auth/register?auctionItemId=${auctionItem?._id}&customCampaignLink=${customCampaignLink}&conversionSource=auction_item_header`
                  }
                ...
                >
                  {user?.user?._id ? 'Profile' : 'Join Now'}
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className='max-w-7xl mx-auto px-3 sm:px-6 py-12'>
          ...
          <div className='flex items-center space-x-2 text-white/60 text-sm mb-8'>
            <Link to={`/campaigns/${campaign?.campaign?.customCampaignLink}/auction`}>Auction</Link>
          ...
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16'>
            ...
            <div className='space-y-6'>
              ...
              <div className='flex justify-between items-start mb-4'>
                <div
                  className={`text-white px-6 py-3 rounded-2xl font-bold text-lg shadow-xl flex items-center space-x-2 ${(() => {
                    ...
                    const count = isAuction ? auctionItem?.bids?.length || 0 : auctionItem?.instantBuyers?.length || 0;

                    if (count === 0) return 'bg-gradient-to-r from-gray-500 to-gray-600';
                    if (count >= 1 && count <= 2) return 'bg-gradient-to-r from-blue-500 to-purple-600';
                    if (count >= 3 && count <= 4) return 'bg-gradient-to-r from-orange-500 to-red-500';
                    if (count >= 5 && count <= 9) return 'bg-gradient-to-r from-red-500 to-pink-500';
                ...
                >
                  {(() => {
                    ...
                    if (count === 0) {
                      if (campaign?.campaign?.auction?.settings?.status === 'UPCOMING') {
                        return (
                          <>
                            <span>✨</span>
              ...
              <div className='relative group'>
                ...
                <img
                  ...
                  className='w-full h-96 lg:h-[500px] object-cover rounded-3xl shadow-2xl border border-white/20'
                />

                {/* Image Navigation */}
                <div
                  className={`${
                    campaign?.campaign?.auction?.items?.length >= 2 ? 'block' : 'hidden'
                ...
                >
                  <button
                    onClick={() => setSelectedImage(Math.min((auctionItem?.photos?.length || 1) - 1, selectedImage + 1))}
                    className='w-12 h-12 bg-black/50 hover:bg-black/70 text-white rounded-full flex items-center justify-center transition-all backdrop-blur-sm'
              ...
              <div className='grid grid-cols-5 gap-3'>
                {auctionItem?.photos?.map((photo: any, index: number) => (
                  ...
                  >
                    <img src={photo?.url} alt={`View ${index + 1}`} className='w-full h-16 object-cover' />
                    {selectedImage === index && <div className='absolute inset-0 bg-yellow-400/20 border-2 border-yellow-400 rounded-xl'></div>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className='space-y-8'>
              {/* Title Section */}
              <div>
                <h1 className='text-4xl lg:text-5xl font-bold text-white mb-4 leading-tight'>{auctionItem?.name}</h1>
              ...
              <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
                <div className='bg-gradient-to-br from-green-400/20 to-emerald-500/20 backdrop-blur-xl rounded-2xl p-6 border border-green-400/30'>
                  ...
                  <div className='text-4xl font-bold text-white mb-1'>${auctionItem?.buyNowPrice || auctionItem?.currentBid}</div>
                  <div className='text-green-200 text-sm space-y-1'>
                    <div>{auctionItem?.buyNowPrice ? 'Instant purchase' : `${auctionItem?.bids?.length || 0} bids`}</div>
                    {/* Add minimum bid for auction items */}
                    {!auctionItem?.buyNowPrice && auctionItem?.minimumBid && (
                      <div className='text-green-300/80 text-xs'>Min bid: ${auctionItem.minimumBid.toLocaleString()}</div>
                    )}
                  </div>
                </div>
                ...
                <div className='flex items-center space-x-3 mb-3'>
                  ...
                  <div>
                    <h4 className='text-white font-semibold'>
                      {(auctionItem?.shippingCosts || 0) > 0 ? `$${auctionItem?.shippingCosts}` : 'Free'} Shipping
                    </h4>
        ...
        <div className='lg:hidden fixed bottom-0 left-0 right-0 bg-white/10 backdrop-blur-xl border-t border-white/20 p-4 z-50'>
          <div className='max-w-7xl mx-auto space-y-3'>
            {/* Navigation Row */}
            <div className={`${campaign?.campaign?.auction?.items?.length >= 2 ? 'block' : 'hidden'} flex items-center justify-between`}>
              {prevItem && (
                <button
                  onClick={() => navigateToItem(prevItem._id)}
            ...
            <div className='flex items-center justify-between'>
              <div className='text-white'>
                ...
                <div className='text-2xl font-bold'>${auctionItem?.buyNowPrice || auctionItem?.currentBid}</div>
              </div>
              <button
