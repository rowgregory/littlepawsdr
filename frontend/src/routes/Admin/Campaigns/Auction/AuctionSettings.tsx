...
const AuctionSettings = () => {
  ...
  const { inputs, handleSwitch, handleInput } = useAuctionSettingsForm(
    campaign?.campaign?.auction
  );

  const todayInNewYork = new Date().toLocaleString('en-US', {
    timeZone: 'America/New_York',
  });
  const today = formatDateForCalandar(new Date(todayInNewYork));
      ...
      <form className='grid gap-10 py-4 px-2.5 md:p-8'>
        <section>
          ...
          <p className='font-Matter-Light'>
            Choose the start and end time for your auction
          </p>
          ...
          <div className='flex flex-col mt-8'>
            ...
            <p className='text-sm font-Matter-Light'>
              When enabled, your auction will be publicly viewable from your campaign.
              When disabled your auction will not be displayed on your campaign.
            </p>
          </div>
        </section>
        <section>
          ...
          <div className='border-[1px] border-[#3366ff] rounded-xl p-3 flex items-baseline bg-[#eff4ff]'>
            <label className='flex mb-0'>
              ...
              <div>
                ...
                <p className='text-sm font-Matter-Light'>
                  Send a payment request to the winning bidder after an item has been won
                  or purchased.
                </p>
              </div>
            </label>
          ...
          <button
            ...
            onClick={(e: any) =>
              genericUpdateAuction(e, 'settings', {
                ...
                hasBegun: hasAuctionStarted(
                  formatDateForEstTimezone(inputs.startDate, 13, 0)
                ),
                hasEnded: false,
                auctionStatus: hasAuctionStarted(
                  formatDateForEstTimezone(inputs.startDate, 13, 0)
                )
                  ? 'Bidding closes'
                  : 'Bidding opens',
