...
import { OneTimeDonationForm, OneTimeDonationProgressTracker } from '../../components/donate/OneTimeDonation';
import { MonthlyDonationForm, MonthlyDonationProgressTracker } from '../../components/donate/MonthlyDonation';
        ...
        <div className='flex flex-col mx-auto relative bg-slate-100 min-h-[calc(100vh-732px)] md:min-h-[calc(100vh-780px)] pb-40'>
          <div className='grid grid-cols-12 gap-4 md:gap-8  sm:px-6 mt-[-192px] md:mt-[-240px] max-w-screen-xl w-full mx-auto h-full'>
            <div className='col-span-12 lg:col-span-8 pt-[24px] md:pt-[48px] w-full'>
              <h1 className='text-3xl px-3 md:text-5xl font-Matter-Bold text-[#fff] mb-3 md:mb-4'>Donate to LPDR and Help Dachshunds</h1>
              <div className='flex items-center md:gap-3.5'>
                ...
                >
                  ONE TIME
                  <span className={`hidden sm:block ${type === 'one-time' ? 'text-teal-500' : 'text-[#fff]'} text-xl`}>&nbsp;DONATION</span>
                </div>
                <div
                ...
                >
                  MONTHLY
                  <span className={`hidden sm:block ${type === 'monthly' ? 'text-teal-500' : 'text-[#fff]'} text-xl`}>&nbsp;DONATION</span>
                </div>
              </div>
              <div className='w-full pt-3 px-4 md:px-6 pb-12 bg-white'>
                ...
                <OneTimeDonationForm type={type} step={step} setStep={setStep} setOpenModal={setOpenModal} />
            ...
            <div className='col-span-12 lg:col-span-4 flex flex-col sm:flex-row lg:flex-col w-full'>
              <img src={Donate1} alt='Donate to LPDR' className='max-w-none sm:max-w-80 lg:max-w-none aspect-square object-cover' />
              <p className='bg-white px-6 p-8 font-QLight'>
                Little Paws Dachshund Rescue is a dedicated organization committed to rescuing, rehabilitating, and rehoming dachshunds in need. As a
                passionate advocate for these beloved dogs, our mission is to provide them with the care and support they require for a second chance
                at a happy life. Through our efforts, we strive to prevent cruelty and neglect, offering a safe haven for dachshunds who have been
                abandoned, abused, or surrendered. Your generous contributions enable us to continue our vital work, making a significant impact on
                the lives of dachshunds in need. Thank you for choosing to support Little Paws Dachshund Rescue and for helping us make a difference
