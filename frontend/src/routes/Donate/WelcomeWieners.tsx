...
const WelcomeWieners = () => {
  ...
  const handleRandomItem = () => {
    const randomIndex = Math.floor(Math.random() * welcomeWiener?.welcomeWieners?.length);
      ...
      <div className='min-h-[calc(100vh-540px)] mt-[65px] pb-20 w-full'>
        <div className='mx-auto w-full'>
          ...
          <div className='w-full mx-auto max-w-screen-xl px-3 flex flex-col'>
            {welcomeWiener?.welcomeWieners?.length > 0 && (
              <button
                style={{
            ...
            <div className='grid grid-cols-12 gap-4 md:gap-7 '>
              {welcomeWiener?.welcomeWieners?.map(
                (wiener: any, i: number) =>
                  (wiener?.isLive || auth?.user?.isAdmin) && (
                    <Link
                      id={`${i}`}
                      key={wiener?._id}
                      className={`col-span-6 sm:col-span-6 md:col-span-4 lg:col-span-3 gap-2 relative aspect-square ${
                        i === randIndex
                    ...
                    >
                      <img
                        ...
                        className={`rounded-md duration-200 aspect-square object-cover bg-gray-100 ${
                          i === randIndex ? 'p-2.5' : ''
                        }`}
                        alt={`LPDR wiener - ${wiener?.name}`}
                      />
                      <p
                        className={`font-Mater-Bold absolute duration-200 text-[#fff] bottom-2 left-1/2 transform -translate-x-1/2 whitespace-nowrap ${
                          i === randIndex ? 'text-2xl' : 'text-lg'
                        }`}
                      >
                        {wiener?.name}
                      </p>
